...
assert original_packet_to_compare.raw_bytes == reconstructed_packet_to_compare.raw_bytes
print(
...
for layer_name, layer_arr in header_arrs.items():
    cv2.imwrite(
        str(out_pth / f"session_layer_{layer_name}.png"), layer_arrs[layer_name]
    )
    cv2.imwrite(
        str(out_pth / f"session_header_{layer_name}.png"), new_header_arrs[layer_name]
    )
